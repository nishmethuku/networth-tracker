  ...
  return (
    <Card title="Recurring investment (SIP)">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "0.5rem", marginBottom: "1rem" }}>
        ...
        <button
          onClick={() => setEditing(true)}
          style={{ background: "none", border: "none", color: "var(--primary)", cursor: "pointer", fontSize: "0.8125rem", fontWeight: 500 }}
        >
          Edit
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "0.25rem" }}>
            Assumes {(projection.assumed_annual_rate * 100).toFixed(1)}% annual return (this holding's current XIRR, or 8% if not yet
            available) and {formatCurrencyForDisplay(projection.total_contributions, holding.currency)} in future contributions. Not a
            guarantee.
          </div>
        </>
      ) : null}
